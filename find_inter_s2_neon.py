...
matches = ee.List.sequence(0, neon_ids.size().subtract(1)) \
    ...
    .filter(ee.Filter.notNull(['ID_SENTINEL']))  # Filtra los casos donde no hay coincidencias
...
for feature in results_json['features']:
    ...
    sentinel_ids = properties['ID_SENTINEL']
...
df_final = pd.DataFrame(data, columns=['ID_NEON', 'ID_SENTINEL'])
